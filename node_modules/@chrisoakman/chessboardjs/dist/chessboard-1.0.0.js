  ...
  function constructor (containerElOrString, config) {
    ...
    function error (code, msg, obj) {
      ...
      if (config.showErrors === 'console' && typeof console === 'object' && typeof console.log === 'function') {
        ...
        if (arguments.length >= 2) console.log(obj)
        return
      }
    ...
    function buildPieceImgSrc (piece) {
      ...
      if (isString(config.pieceTheme)) return interpolateTemplate(config.pieceTheme, {piece: piece})
    ...
    function buildSparePiecesHTML (color) {
      ...
      if (color === 'black') pieces = ['bK', 'bQ', 'bR', 'bB', 'bN', 'bP']
      var html = ''
    ...
    function animateSquareToSquare (src, dest, piece, completeFn) {
      ...
      var opts = { duration: config.moveSpeed, complete: onFinishAnimation1 }
    ...
    function animateSparePieceToSquare (piece, dest, completeFn) {
      ...
      var opts = { duration: config.moveSpeed, complete: onFinishAnimation2 }
    ...
    function doAnimations (animations, oldPos, newPos) {
      ...
      function onFinishAnimation3 () {
        ...
        drawPositionInstant()
        if (isFunction(config.onMoveEnd)) config.onMoveEnd(deepCopy(oldPos), deepCopy(newPos))
      }
    ...
    function setCurrentPosition (position) {
      ...
      if (isFunction(config.onChange)) config.onChange(oldPos, newPos)
    ...
    function snapbackDraggedPiece () {
      if (draggedPieceSource === 'spare') { trashDraggedPiece(); return }
      ...
      var opts = { duration: config.snapbackSpeed, complete: complete }
    ...
    function dropDraggedPieceOnSquare (square) {
      ...
      function onAnimationComplete () {
        drawPositionInstant()
        $draggedPiece.css('display', 'none')
        if (isFunction(config.onSnapEnd)) config.onSnapEnd(draggedPieceSource, square, draggedPiece)
      }
      var opts = { duration: config.snapSpeed, complete: onAnimationComplete }
    ...
    function updateDraggedPiece (x, y) {
      $draggedPiece.css({ left: x - squareSize / 2, top: y - squareSize / 2 })
    ...
    function stopDraggedPiece (location) {
      var action = 'drop'
      if (location === 'offboard' && config.dropOffBoard === 'snapback') action = 'snapback'
      if (location === 'offboard' && config.dropOffBoard === 'trash') action = 'trash'
      if (isFunction(config.onDrop)) {
        var newPosition = deepCopy(currentPosition)
        if (draggedPieceSource === 'spare' && validSquare(location)) newPosition[location] = draggedPiece
        if (validSquare(draggedPieceSource) && location === 'offboard') delete newPosition[draggedPieceSource]
        ...
        if (result === 'snapback' || result === 'trash') action = result
      }
      if (action === 'snapback') {
    ...
    widget.clear = function (useAnimation) { widget.position({}, useAnimation) }
    ...
    widget.fen = function () { return widget.position('fen') }

    widget.flip = function () { return widget.orientation('flip') }
    ...
    widget.move = function () {
      ...
      for (var i = 0; i < arguments.length; i++) {
        ...
        if (!validMove(arguments[i])) { error(2826, 'Invalid move passed to the move method.', arguments[i]); continue }
    ...
    widget.position = function (position, useAnimation) {
      ...
      if (isString(position) && position.toLowerCase() === 'fen') return objToFen(currentPosition)
      if (isString(position) && position.toLowerCase() === 'start') position = deepCopy(START_POSITION)
      if (validFen(position)) position = fenToObj(position)
      if (!validPositionObject(position)) { error(6482, 'Invalid value passed to the position method.', position); return }
    ...
    widget.start = function (useAnimation) { widget.position('start', useAnimation) }

    // -------------------------------------------------------------------------
    // Browser Events
    // -------------------------------------------------------------------------

    function stopDefault (evt) { evt.preventDefault() }
    ...
    function mousemoveWindow (evt) {
      if (isDragging) updateDraggedPiece(evt.pageX, evt.pageY)
    }
    ...
    function mouseenterSquare (evt) {
      ...
      if (!validSquare(square)) return
      var piece = false
      if (currentPosition.hasOwnProperty(square)) piece = currentPosition[square]
...
// Footer reset button — restores player to white, board to white orientation
$('.footer button').eq(0).on('click', function() {
